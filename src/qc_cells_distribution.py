...
def open_cluster_file(sample_id, barcode_root=None):
    if barcode_root:
        candidates = [
            Path(barcode_root) / sample_id / "mapping" / f"{sample_id}_filter40_barcode.txt",
            Path(barcode_root) / sample_id / f"{sample_id}_filter40_barcode.txt",
            Path(barcode_root) / f"{sample_id}_filter40_barcode.txt",
        ]
        barcode_file = None
        ...
        if barcode_file is None:
            raise FileNotFoundError(
                f"Could not find barcode file for sample '{sample_id}' under {barcode_root}"
            )
    else:
        # Legacy fallback
        ...
        barcode_file = Path(f"{bam_location}/{sample_id}_filter40_barcode.txt")
...
def resolve_bam_matrix_path(bam_arg, sample_id):
    path = Path(bam_arg)
    if path.is_file():
        return path

    candidates = [
        path / sample_id / f"{sample_id}.gene_count.mx",
        path / sample_id / "gene_count.mx",
        path / f"{sample_id}.gene_count.mx",
    ]
    for c in candidates:
        if c.exists():
            return c
    ...
    raise FileNotFoundError(
        f"Could not resolve BAM-derived matrix for sample '{sample_id}' from --bam={bam_arg}. "
        f"Tried: {', '.join(str(x) for x in candidates)}"
    )


def parse_coverage_csv(text):
...
def get_args():
    ...
    parser.add_argument('--bam', required = False, default = '', type=str,
                        help=(
                            "Path to BAM-derived gene-count matrix input generated by "
                            "qc_create_count_matrix.py. Can be either a single .mx file "
                            "or a directory containing per-sample outputs."
                        ))
    parser.add_argument('--barcode-root', required = False, default = '', type=str,
                        help=(
                            "Optional root path to resolve barcode files for each sample "
                            "(<root>/<sample>/mapping/<sample>_filter40_barcode.txt). "
                            "Used to map raw barcode columns to bcX."
                        ))
...
def main():
    ...
    method = args.method
    coverage_to_plot = parse_coverage_csv(args.coverage)
    ...
    if use_bam:
        ...
        batch_id += '_bam'
        if Path(args.bam).is_file() and len(sample_list) > 1:
            raise ValueError(
                "--bam points to a single file but multiple samples were provided. "
                "Use one sample ID or pass a directory containing per-sample matrices."
            )
    ...
    for sample_id in sample_list:
        ...
        barcode_dict = open_cluster_file(sample_id, args.barcode_root or None)
        if use_bam:
            filename = str(resolve_bam_matrix_path(args.bam, sample_id))
        else:
            from common import workdir
